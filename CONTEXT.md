...
**เรียงลำดับรายการ "คณะที่คะแนนถึง"** (`SORTS` ใน ResultsPage): ส่วนต่างมาก→น้อย (default) · ส่วนต่างน้อย→มาก · อันดับมหาวิทยาลัย (SCImago SIR 2026 จาก `uniRanking.js` — มหาลัยไม่ติด SIR = Infinity เรียงท้าย, tie-break ด้วยส่วนต่าง). อัปเดตปี SIR ใหม่: แก้ map ใน `uniRanking.js` (scimagoir.com ติด Cloudflare ดึงตรงไม่ได้ — ใช้ Wayback Machine snapshot ของหน้า `rankings.php?sector=Higher+educ.&country=THA` แกะ `<tr>` ได้ตารางครบ 42 สถาบัน)
...
src/lib/uniRanking.js     อันดับมหาวิทยาลัย SCImago SIR 2026 (UNI_SIR_RANK keyed ด้วย university_id, 34 มหาลัย) → ตัวเลือกเรียง "อันดับมหาวิทยาลัย" ในหน้าผล
